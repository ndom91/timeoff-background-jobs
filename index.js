...
const writeDays = async (days, email) => {
  ...
  const result = await mysql.query(readQuery)
	...
	if (result[0]) {
		let daysLastYear = result[0].resturlaubVorjahr
		let daysEarned = result[0].jahresurlaubInsgesamt
		let daysSpent = parseFloat(result[0].jahresUrlaubAusgegeben)
		let daysAvailable = result[0].restjahresurlaubInsgesamt
		let daysRequested = result[0].beantragt
		let daysLeft = result[0].resturlaubJAHR
     ...
     // DEBUG
     console.log(`${email} - had ${daysLastYear} - earned ${daysEarned} - now das ${daysAvailable} days`)

		// if (email === 'ndomino@newtelco.de') {
